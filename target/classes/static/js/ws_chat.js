    ...
    if (jsonarray != "") {
        ...
        $.each(jsonarray, function (i, msg) {
            ...
            if ((relation_type == 0) && (owner_uid == sender_id)) { //自己发的
                ...
                li_msg.attr("avatar", 'url(/image/' + owner_uid_avatar + ')');
        ...
        $("<style id='self-chat-li-style'>#self-chat-li:before{background-image:url('/image/" + owner_uid_avatar + "')}</style>").appendTo('head');
        $("<style id='other-chat-li-style'>#other-chat-li:before{background-image:url('/image/" + other_uid_avatar + "')}</style>").appendTo('head');
    }
}
...
function handleSendMsgResp(resp) {
    ...
    li_current.attr("avatar", 'url(/image/' + sender_avatar + ')');
    ...
    if (flag1 == false) {
        var td_images = "<td><img width='50px' src='/image/" + jsonContent.otherUidAvatar + "'/></td>";
...
function handleReceivedMsg(pushedMsg) {
    ...
    li_current.attr("avatar", 'url(/image/' + jsonRecipient.ownerUidAvatar + ')');
    ul_pane.append(li_current);
    $('.chat-thread').animate({scrollTop: $('.chat-thread').prop("scrollHeight")}, 10);
    ...
    if (flag2 == false) {
        var td_images = "<td><img width='50px' src='/image/" + jsonRecipient.ownerUidAvatar + "'/></td>";
